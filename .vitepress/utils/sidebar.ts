/**
 * Copied from https://github.com/jooy2/vitepress-sidebar/blob/master/package.json
 * 
 * MIT License
 *
 * Fixes an issue with numeric ordering of the sidebar using the frontmatter 
 * `order` field
 */
  ...
  private static generateSidebarItem(
    ...
    if (options.sortMenusByFrontmatterOrder) {
      sidebarItems = VitePressSidebar.sortByObjectKey({
        ...
        key: 'order',
        desc: options.sortMenusOrderByDescending,
        // This option is missing in the main package
        numerically: true
      });
